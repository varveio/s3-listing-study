...
import measure  # type: ignore[import-not-found]
...
def test_worker_requirement_versions_match_repository_lock() -> None:
    ...
    for line in (ROOT / "simple/requirements-worker.txt").read_text().splitlines():
...
def test_timeout_kills_process_group_and_records_cgroup_oom(tmp_path: Path) -> None:
    ...
    old = os.environ.get("SIMPLE_CGROUP_DIR")
    os.environ["SIMPLE_CGROUP_DIR"] = str(cgroup)
    try:
        ...
        if old is None:
            os.environ.pop("SIMPLE_CGROUP_DIR", None)
        else:
            os.environ["SIMPLE_CGROUP_DIR"] = old
...
def test_recursive_upload_preserves_native_paths(
    ...
    uploaded: list[str] = []
    monkeypatch.setattr(measure.gcs, "upload_file", lambda _path, uri: uploaded.append(uri))
