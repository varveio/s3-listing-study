...
def job_document() -> dict[str, object]:
    values = {
        ...
        "--shared-base-uri": "registry/shared@sha256:" + "8" * 64,
        "--shared-base-digest": "sha256:" + "8" * 64,
        "--shared-base-source-sha256": "7" * 64,
        "--toolbox-manifest-sha256": "9" * 64,
        "--tool-parent-image": "registry/parent@sha256:" + "c" * 64,
        "--tool-version": "1",
...
def test_report_refuses_poisoned_result_and_verify_bindings(
    ...
    result: dict[str, object] = {
        ...
        "tool_parent_image": "registry/parent@sha256:" + "c" * 64,
        "shared_base_uri": "registry/shared@sha256:" + "8" * 64,
        "shared_base_digest": "sha256:" + "8" * 64,
        "shared_base_source_sha256": "7" * 64,
...
def test_report_finality_requires_terminal_provider_states() -> None:
    accepted = [{"job_state": "ACCEPTED_FAILED", "evidence_state": "UNAVAILABLE"}]
...
def test_report_recomputes_verification_instead_of_trusting_claimed_diff(tmp_path: Path) -> None:
    ...
    for tool in ("actual", "reference"):
        ...
        result = {
            ...
            "stdout_gz_sha256": report.verify.sha256_of(stdout),
            "stderr_gz": stderr.name,
            "stderr_gz_sha256": report.verify.sha256_of(stderr),
            "native_manifest": {},
        }
        (leaf / "result.json").write_text(json.dumps(result))
        destinations[tool] = (destination, leaf, result)

    code, verification = report.verify.verify_leaves(
        tool="actual",
        bucket="bucket",
        prefix="",
        mode="mode",
