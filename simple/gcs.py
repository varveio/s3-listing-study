...
Replaces round 1/2's gsutil subprocess calls outright (owner correction):
...
def blob_exists(uri: str) -> bool:
    ...
    return client().bucket(bucket_name).blob(name).exists()


def download_bytes(uri: str) -> bytes:
    bucket_name, name = parse_gs_uri(uri)
    return client().bucket(bucket_name).blob(name).download_as_bytes()
