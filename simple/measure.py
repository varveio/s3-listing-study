...
and GCP Batch's job status completely. It does NOT do: secret scanning of
captured output, create-only ("never overwrite") upload preconditions,
TwinStamp evidence sealing, disk sampling, or SIGTERM/grace-period handling.
It DOES keep two properties that are cheap here and expensive to lose (see
README.md's "The minimum rigor we kept"): every invocation uploads to its own
uuid4 leaf under --destination rather than a shared path, so two launches of
the same case can never overwrite each other's artifacts; and result.json is
written atomically and uploaded last, so a leaf missing it is legible as an
incomplete attempt rather than silently read as a complete, empty one.
