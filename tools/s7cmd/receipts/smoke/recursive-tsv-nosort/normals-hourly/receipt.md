# Smoke receipt — `s7cmd` / mode `recursive-tsv-nosort`
...
| | |
| --- | --- |
| Date (UTC) | 2026-07-17T08:12:19Z |
| Exit code | `0` |
| Wall-clock | 0.579s (container lifetime, StartedAt→FinishedAt) |
...
| | | |
| --- | --- | --- |
| `peak_rss` | 22.4 MB | `VmHWM` of the container's main process, 10 successful samples. **Main process only** — a multi-process fan-out mode's children are not included. |
| `cgroup_peak_mem` | 10.7 MB | cgroup v2 `memory.peak`, whole container tree, 10 successful samples. **Page cache and kernel/socket memory included. Never present this as RSS.** |
...
The tool's own counter reported **api_calls=17** (`Listing pipeline completed api_calls=17`, s7cmd `-vv` debug on stderr — its internal `AtomicU64` incremented once per `ListObjectsV2`/`ListObjectVersions` page).

## Raw output

- stdout: external — `receipts/s7cmd/recursive-tsv-nosort.noaa-normals-pds.normals-hourly_.anonymous.stdout.txt` (299225 bytes, sha256 `e9b3d5c7d37f0dc4a14190b3c53cc429fbd2add2be619dd174b5ec922eb1dfd6`) — redacted and scanned before hashing; published as a release asset at publication
- stderr: inline — `stderr.txt` (686 bytes, sha256 `e08768f4ef285d6806f99233362bec359ffb12e9bfe7f72d5576e44c9f35bbe5`)
