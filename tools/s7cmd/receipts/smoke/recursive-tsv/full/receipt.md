...
| | |
| --- | --- |
| Date (UTC) | 2026-07-17T08:10:40Z |
| Exit code | `0` |
| Wall-clock | 2.627s (container lifetime, StartedAt→FinishedAt) |
...
| | | |
| --- | --- | --- |
| `peak_rss` | 120.8 MB | `VmHWM` of the container's main process, 47 successful samples. **Main process only** — a multi-process fan-out mode's children are not included. |
| `cgroup_peak_mem` | 110.0 MB | cgroup v2 `memory.peak`, whole container tree, 48 successful samples. **Page cache and kernel/socket memory included. Never present this as RSS.** |
...
The tool's own counter reported **api_calls=204** (`Listing pipeline completed api_calls=204`, s7cmd `-vv` debug on stderr — its internal `AtomicU64` incremented once per `ListObjectsV2`/`ListObjectVersions` page).

## Raw output

- stdout: external — `receipts/s7cmd/recursive-tsv.noaa-normals-pds.full.anonymous.stdout.txt` (17504711 bytes, sha256 `43cd4fe9325a6b19d4242245b3bb0b3ebacb3e1a67d2b560eadff417c73834ff`) — redacted and scanned before hashing; published as a release asset at publication
- stderr: inline — `stderr.txt` (866 bytes, sha256 `9c327c258fdaa641f69e8240d420470d65096cffe550ce3afe2783f9b8339a67`)
