...
| | |
| --- | --- |
| Date (UTC) | 2026-07-17T08:12:09Z |
| Exit code | `0` |
| Wall-clock | 1.138s (container lifetime, StartedAt→FinishedAt) |
...
| | | |
| --- | --- | --- |
| `peak_rss` | 26.4 MB | `VmHWM` of the container's main process, 20 successful samples. **Main process only** — a multi-process fan-out mode's children are not included. |
| `cgroup_peak_mem` | 14.5 MB | cgroup v2 `memory.peak`, whole container tree, 20 successful samples. **Page cache and kernel/socket memory included. Never present this as RSS.** |
...
The tool's own counter reported **api_calls=10** (`Listing pipeline completed api_calls=10`, s7cmd `-vv` debug on stderr — its internal `AtomicU64` incremented once per `ListObjectsV2`/`ListObjectVersions` page).

## Raw output

- stdout: external — `receipts/s7cmd/recursive-tsv.noaa-normals-pds.normals-annualseasonal_1981-2010_access_.anonymous.stdout.txt` (1227084 bytes, sha256 `04b51de9cdc49e68879ec39ac5cb9734d04dac58649670f546c89b52cae487c1`) — redacted and scanned before hashing; published as a release asset at publication
- stderr: inline — `stderr.txt` (901 bytes, sha256 `b64a817dc5af3046e33d9eef41764ceda543c1afd843a05598df651547fe59b1`)
