# Smoke receipt — `s7cmd` / mode `recursive-tsv`
...
| | |
| --- | --- |
| Date (UTC) | 2026-07-17T08:12:06Z |
| Exit code | `0` |
| Wall-clock | 1.928s (container lifetime, StartedAt→FinishedAt) |
...
| Tool version | `s7cmd 1.5.0` — caller-supplied |

## Invocation

```sh
docker run -d --network host -e AWS_EC2_METADATA_DISABLED=true -e TZ=UTC -e AWS_ACCESS_KEY_ID= -e AWS_SECRET_ACCESS_KEY= -e AWS_SESSION_TOKEN= -e AWS_SECURITY_TOKEN= -e AWS_CONTAINER_CREDENTIALS_RELATIVE_URI= -e AWS_CONTAINER_CREDENTIALS_FULL_URI= -e AWS_CONTAINER_AUTHORIZATION_TOKEN= -e AWS_ROLE_ARN= -e AWS_SHARED_CREDENTIALS_FILE=/nonexistent-by-harness -e AWS_CONFIG_FILE=/nonexistent-by-harness -e AWS_WEB_IDENTITY_TOKEN_FILE=/nonexistent-by-harness s7cmd@sha256:07091182512e74cde4bb897a97b1fc9a586757560c5008ae8c701d7fdb6974da ls -r -vv --disable-color-tracing --tsv --show-storage-class --show-etag --max-parallel-listings 16 --target-no-sign-request --target-region us-east-1 s3://noaa-normals-pds/normals-monthly/1991-2020/
```
...
| Region | `us-east-1` |
| Prefix scope | `normals-monthly/1991-2020/`normals-monthly/1991-2020/ |
...
| | | |
| --- | --- | --- |
| `peak_rss` | 30.5 MB | `VmHWM` of the container's main process, 35 successful samples. **Main process only** — a multi-process fan-out mode's children are not included. |
| `cgroup_peak_mem` | 18.8 MB | cgroup v2 `memory.peak`, whole container tree, 35 successful samples. **Page cache and kernel/socket memory included. Never present this as RSS.** |
...
The tool's own counter reported **api_calls=19** (`Listing pipeline completed api_calls=19`, s7cmd `-vv` debug on stderr — its internal `AtomicU64` incremented once per `ListObjectsV2`/`ListObjectVersions` page).

## Raw output

- stdout: external — `receipts/s7cmd/recursive-tsv.noaa-normals-pds.normals-monthly_1991-2020_.anonymous.stdout.txt` (1837199 bytes, sha256 `a3541eda13f81f0e4b2f1e19d777afa2a67e73cc0845bf93fd8c17e3695f9726`) — redacted and scanned before hashing; published as a release asset at publication
- stderr: inline — `stderr.txt` (889 bytes, sha256 `8b2142b898ff8fec43f85ac740d2d381dfa545a107d37cbdb098938d72d3fc2d`)
