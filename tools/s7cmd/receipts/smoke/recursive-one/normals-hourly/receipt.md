# Smoke receipt — `s7cmd` / mode `recursive-one`
...
| | |
| --- | --- |
| Date (UTC) | 2026-07-17T08:12:15Z |
| Exit code | `0` |
| Wall-clock | 0.489s (container lifetime, StartedAt→FinishedAt) |
...
| | | |
| --- | --- | --- |
| `peak_rss` | 23.3 MB | `VmHWM` of the container's main process, 8 successful samples. **Main process only** — a multi-process fan-out mode's children are not included. |
| `cgroup_peak_mem` | 11.9 MB | cgroup v2 `memory.peak`, whole container tree, 8 successful samples. **Page cache and kernel/socket memory included. Never present this as RSS.** |
...
- stdout: external — `receipts/s7cmd/recursive-one.noaa-normals-pds.normals-hourly_.anonymous.stdout.txt` (118254 bytes, sha256 `e71289eb3c4e0dc0f4d4f163e2b22414d0a93d5876fe5e11535490e5fd50dc54`) — redacted and scanned before hashing; published as a release asset at publication
- stderr: inline — `stderr.txt` (876 bytes, sha256 `ddf12a6cfe49124e34fd1694b836c3876a976615dd48ef58a72392c854af322c`)
