...
| Exit code | `0` |
| Wall-clock | 0.177s (container lifetime, StartedAt→FinishedAt) |
...
```sh
docker run -d --network host -e AWS_EC2_METADATA_DISABLED=true -e TZ=UTC -e AWS_ACCESS_KEY_ID= -e AWS_SECRET_ACCESS_KEY= -e AWS_SESSION_TOKEN= -e AWS_SECURITY_TOKEN= -e AWS_CONTAINER_CREDENTIALS_RELATIVE_URI= -e AWS_CONTAINER_CREDENTIALS_FULL_URI= -e AWS_CONTAINER_AUTHORIZATION_TOKEN= -e AWS_ROLE_ARN= -e AWS_SHARED_CREDENTIALS_FILE=/nonexistent-by-harness -e AWS_CONFIG_FILE=/nonexistent-by-harness -e AWS_WEB_IDENTITY_TOKEN_FILE=/nonexistent-by-harness s7cmd@sha256:07091182512e74cde4bb897a97b1fc9a586757560c5008ae8c701d7fdb6974da ls -vv --disable-color-tracing --tsv --show-storage-class --show-etag --target-no-sign-request --target-region us-east-1 s3://noaa-normals-pds/normals-hourly/
```
...
| `cgroup_peak_mem` | 7.7 MB | cgroup v2 `memory.peak`, whole container tree, 3 successful samples. **Page cache and kernel/socket memory included. Never present this as RSS.** |
...
- stdout: inline — `stdout.txt` (187 bytes, sha256 `9077636731b250414a194c944aa82de09485f69ffbf02eaf8b64142a34fb7fb6`)
- stderr: inline — `stderr.txt` (844 bytes, sha256 `3eae6aca2ae35565430ce0946996e455f8cffebf652b458df0e7ad49374dd495`)
