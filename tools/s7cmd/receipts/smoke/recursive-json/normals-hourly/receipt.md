# Smoke receipt — `s7cmd` / mode `recursive-json`
...
| | |
| --- | --- |
| Date (UTC) | 2026-07-17T08:12:14Z |
| Exit code | `0` |
| Wall-clock | 0.518s (container lifetime, StartedAt→FinishedAt) |
...
| | | |
| --- | --- | --- |
| `peak_rss` | 23.4 MB | `VmHWM` of the container's main process, 9 successful samples. **Main process only** — a multi-process fan-out mode's children are not included. |
| `cgroup_peak_mem` | 11.9 MB | cgroup v2 `memory.peak`, whole container tree, 9 successful samples. **Page cache and kernel/socket memory included. Never present this as RSS.** |
...
- stdout: external — `receipts/s7cmd/recursive-json.noaa-normals-pds.normals-hourly_.anonymous.stdout.txt` (612752 bytes, sha256 `3c3c1a3b8d202ca036328479fc56cb664d72e34930037eaeef8664ee9fda700a`) — redacted and scanned before hashing; published as a release asset at publication
- stderr: inline — `stderr.txt` (876 bytes, sha256 `8b572c7d4219c185349a4ac8e198098462114f56f588c406d4885bf7a94afdad`)
