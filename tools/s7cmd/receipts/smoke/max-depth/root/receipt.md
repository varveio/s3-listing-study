# Smoke receipt — `s7cmd` / mode `max-depth`
...
| | |
| --- | --- |
| Date (UTC) | 2026-07-17T08:12:18Z |
| Exit code | `0` |
| Wall-clock | 0.182s (container lifetime, StartedAt→FinishedAt) |
...
| | | |
| --- | --- | --- |
| `peak_rss` | 19.1 MB | `VmHWM` of the container's main process, 3 successful samples. **Main process only** — a multi-process fan-out mode's children are not included. |
| `cgroup_peak_mem` | 8.2 MB | cgroup v2 `memory.peak`, whole container tree, 3 successful samples. **Page cache and kernel/socket memory included. Never present this as RSS.** |
...
The tool's own counter reported **api_calls=1** (`Listing pipeline completed api_calls=1`, s7cmd `-vv` debug on stderr — its internal `AtomicU64` incremented once per `ListObjectsV2`/`ListObjectVersions` page).

## Raw output

- stdout: inline — `stdout.txt` (180 bytes, sha256 `b89b54adaddfd0d3a34ec2fede5e29a5ce66043fe2cefcd9203cbf54730d6451`)
- stderr: inline — `stderr.txt` (853 bytes, sha256 `65569d646d215d67290d2c7cc6160c58f2656b62f82f77308da01b54cc0ebbed`)
