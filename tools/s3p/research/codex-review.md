...
3. **High — `normalize.sh` violates its raw-key contract for supported keys.** ``normalize.sh:4`` promises raw key bytes without re-encoding, but `jq @tsv` at line 52 escapes backslashes, tabs, and newlines. Backslash is inside s3p’s supported 95-character alphabet, so this is not limited to deferred Unicode cases. The `ls-long` parser at line 43 also collapses repeated/leading spaces in keys rather than rejecting an unrepresentable record. Either path can silently produce the wrong key.
...
5. **Medium — Empty `ls` results fail normalization.** With `set -o pipefail`, `grep -v '^$'` at ``normalize.sh:41`` exits 1 when input is empty, making a valid empty bucket/prefix an adapter failure instead of successful empty output.
