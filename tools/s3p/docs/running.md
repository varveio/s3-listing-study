...
- `ENTRYPOINT ["s3p"]`; `command.py` preserves that fixed-prefix convention.
- Built image digest
  ...
  note, not a standard wrapper-era receipt]
...
`summarize` empty [the fixture check `check.sh`, not a live wrapper-era
