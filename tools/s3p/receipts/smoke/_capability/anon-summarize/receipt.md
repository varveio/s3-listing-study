# Smoke receipt — `s3p` / mode `summarize`
...
| Exit code | `1` |
| Wall-clock | 0.224s (container lifetime, StartedAt→FinishedAt) |
...
| | | |
| --- | --- | --- |
| `peak_rss` | 65.4 MB | `VmHWM` of the container's main process, 3 successful samples. **Main process only** — a multi-process fan-out mode's children are not included. |
...
before any list completed. Same block as the `ls`/`ls-raw` probes — this receipt
