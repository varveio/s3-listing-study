...
| | |
| --- | --- |
| Date (UTC) | 2026-07-17T12:01:18Z |
| Exit code | `1` |
| Wall-clock | 0.221s (container lifetime, StartedAt→FinishedAt) |
...
| | | |
| --- | --- | --- |
| `peak_rss` | 64.5 MB | `VmHWM` of the container's main process, 3 successful samples. **Main process only** — a multi-process fan-out mode's children are not included. |
| `cgroup_peak_mem` | 30.1 MB | cgroup v2 `memory.peak`, whole container tree, 3 successful samples. **Page cache and kernel/socket memory included. Never present this as RSS.** |
...
s3p **does** expose a list-request counter: its progress heartbeat prints
`listRequests: N` (stderr, once/second) and the final stats object reports
`requests:` / `requestsUsed` [SRC S3Comprehensions.caf:288,525 @ 5a23b22e].
This run emitted **neither** — it aborted at AWS-SDK credential resolution
(`CredentialsProviderError: Could not load credentials from any providers`,
`@aws-sdk/credential-provider-node`) before any progress tick or final-stats
line, so no counter value exists to record here. The stderr does show the
bisection issued **two** concurrent `listObjectsV2` calls
(`startAfter: normals-hourly/` and `startAfter: normals-hourly/O`) before both
failed — request-shape observation, not a completed count. Full request-shape
capture defers to the study's replay-server phase.
