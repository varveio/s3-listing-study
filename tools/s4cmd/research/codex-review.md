...
> 
> - **Low — The UTC explanation is not supported by its `[SRC]` anchor.** `normalize.sh:16` and [research/report.md:214](report.md) say `TZ=UTC` makes the displayed timestamp UTC “by construction.” Upstream merely formats the datetime’s existing fields without timezone conversion (s4cmd.py:1597). UTC, if present, comes from the S3/botocore timestamp, not the container timezone.
> 
