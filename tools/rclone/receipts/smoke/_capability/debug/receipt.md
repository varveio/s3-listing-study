# Smoke receipt — `rclone` / mode `debug`
...
| | |
| --- | --- |
| Date (UTC) | 2026-07-17T12:03:56Z |
| Exit code | `0` |
| Wall-clock | 0.549s (container lifetime, StartedAt→FinishedAt) |
...
```sh
docker run -d --network host -e AWS_EC2_METADATA_DISABLED=true -e TZ=UTC -e AWS_ACCESS_KEY_ID= -e AWS_SECRET_ACCESS_KEY= -e AWS_SESSION_TOKEN= -e AWS_SECURITY_TOKEN= -e AWS_CONTAINER_CREDENTIALS_RELATIVE_URI= -e AWS_CONTAINER_CREDENTIALS_FULL_URI= -e AWS_CONTAINER_AUTHORIZATION_TOKEN= -e AWS_ROLE_ARN= -e AWS_SHARED_CREDENTIALS_FILE=/nonexistent-by-harness -e AWS_CONFIG_FILE=/nonexistent-by-harness -e AWS_WEB_IDENTITY_TOKEN_FILE=/nonexistent-by-harness rclone/rclone@sha256:c61954aaa32328a5486715dd063a81c7879f5195ad3505cd362deddd509dc4a1 lsjson --fast-list --files-only --use-server-modtime --no-mimetype -R -vv --dump headers :s3\,provider=AWS\,region=us-east-1:noaa-normals-pds/normals-hourly/
```
...
| `cgroup_peak_mem` | 19.3 MB | cgroup v2 `memory.peak`, whole container tree, 9 successful samples. **Page cache and kernel/socket memory included. Never present this as RSS.** |
...
- stdout: external — `receipts/rclone/debug.noaa-normals-pds.normals-hourly_.anonymous.stdout.txt` (404259 bytes, sha256 `3c3c637f92e5919439b85ab4eb7c55948871a0c13d04ec0a0dcc120b4986c423`) — redacted and scanned before hashing; published as a release asset at publication
- stderr: inline — `stderr.txt` (5553 bytes, sha256 `bad9cccbb68d30c763b5930d9d21606ddb7c7679b1e41ab787d843b8259db477`)
