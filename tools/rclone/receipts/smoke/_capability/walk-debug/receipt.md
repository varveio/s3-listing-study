# Smoke receipt — `rclone` / mode `walk-debug`
...
| | |
| --- | --- |
| Date (UTC) | 2026-07-17T14:38:49Z |
| Exit code | `0` |
| Wall-clock | 1.244s (container lifetime, StartedAt→FinishedAt) |
...
| | | |
| --- | --- | --- |
| `peak_rss` | 72.9 MB | `VmHWM` of the container's main process, 22 successful samples. **Main process only** — a multi-process fan-out mode's children are not included. |
| `cgroup_peak_mem` | 26.5 MB | cgroup v2 `memory.peak`, whole container tree, 22 successful samples. **Page cache and kernel/socket memory included. Never present this as RSS.** |
...
This probe **is** the request-shape capture for the genuine hierarchical walk
(mode `recursive-walk`). `stderr.txt` (`-vv --dump headers`) records **13
`ListObjectsV2` requests, every one carrying `delimiter=%2F`** — one LIST per
directory node:

- `normals-annualseasonal/1981-2010/` → 1 request (discovers `access/`,
  `archive/`, `doc/` as `CommonPrefixes`)
- `.../access/` → 10 requests (9,839 keys, serial `continuation-token` paging at
  the 1,000-key cap)
- `.../archive/` → 1 request · `.../doc/` → 1 request

No `Authorization` header on any request (unsigned/anonymous confirmed). This is
the delimited per-directory pattern — **contrast `_capability/debug`**, the
`--fast-list` probe, which is a single **undelimited** `continuation-token` chain.
The two probes together prove rclone's two distinct S3 request patterns on the
wire. Request count for a walk is set by the **tree shape**, not `ceil(keys /
list_chunk)`.

## Raw output

- stdout: external — `receipts/rclone/walk-debug.noaa-normals-pds.normals-annualseasonal_1981-2010_.anonymous.stdout.txt` (1453736 bytes, sha256 `76affdbcd17ff0dd27db1dc2186cc7a86c655dd12510e57fe219cc98611eecd0`) — redacted and scanned before hashing; published as a release asset at publication
- stderr: inline — `stderr.txt` (19399 bytes, sha256 `78fbbd510ce8d1c5e8a8d2d93491f9ecb7b0053353282619c156ad288e2bef65`)
