# Smoke receipt — `rclone` / mode `recursive-fastlist`
...
| | |
| --- | --- |
| Date (UTC) | 2026-07-17T12:02:35Z |
| Exit code | `0` |
| Wall-clock | 1.261s (container lifetime, StartedAt→FinishedAt) |
...
```sh
docker run -d --network host -e AWS_EC2_METADATA_DISABLED=true -e TZ=UTC -e AWS_ACCESS_KEY_ID= -e AWS_SECRET_ACCESS_KEY= -e AWS_SESSION_TOKEN= -e AWS_SECURITY_TOKEN= -e AWS_CONTAINER_CREDENTIALS_RELATIVE_URI= -e AWS_CONTAINER_CREDENTIALS_FULL_URI= -e AWS_CONTAINER_AUTHORIZATION_TOKEN= -e AWS_ROLE_ARN= -e AWS_SHARED_CREDENTIALS_FILE=/nonexistent-by-harness -e AWS_CONFIG_FILE=/nonexistent-by-harness -e AWS_WEB_IDENTITY_TOKEN_FILE=/nonexistent-by-harness rclone/rclone@sha256:c61954aaa32328a5486715dd063a81c7879f5195ad3505cd362deddd509dc4a1 lsjson --fast-list --files-only --use-server-modtime --no-mimetype -R :s3\,provider=AWS\,region=us-east-1:noaa-normals-pds/normals-annualseasonal/1981-2010/access/
```
...
| | | |
| --- | --- | --- |
| `peak_rss` | 67.4 MB | `VmHWM` of the container's main process, 22 successful samples. **Main process only** — a multi-process fan-out mode's children are not included. |
| `cgroup_peak_mem` | 21.6 MB | cgroup v2 `memory.peak`, whole container tree, 22 successful samples. **Page cache and kernel/socket memory included. Never present this as RSS.** |
...
- stdout: external — `receipts/rclone/recursive-fastlist.noaa-normals-pds.normals-annualseasonal_1981-2010_access_.anonymous.stdout.txt` (1384511 bytes, sha256 `e9a99a0d16eaa8f716c85d8a591805ca3909c1a5b7e0f8ee27aa07a73517d8da`) — redacted and scanned before hashing; published as a release asset at publication
- stderr: inline — `stderr.txt` (96 bytes, sha256 `b9289b4ecb8d4b3354e78ed103ed46a58a2ae03d2f9230fd81508d69c13609a4`)
