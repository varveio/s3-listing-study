...
| | |
| --- | --- |
| Date (UTC) | 2026-07-17T12:00:33Z |
| Exit code | `0` |
| Wall-clock | 16.954s (container lifetime, StartedAt→FinishedAt) |
...
```sh
docker run -d --network host -e AWS_EC2_METADATA_DISABLED=true -e TZ=UTC -e AWS_ACCESS_KEY_ID= -e AWS_SECRET_ACCESS_KEY= -e AWS_SESSION_TOKEN= -e AWS_SECURITY_TOKEN= -e AWS_CONTAINER_CREDENTIALS_RELATIVE_URI= -e AWS_CONTAINER_CREDENTIALS_FULL_URI= -e AWS_CONTAINER_AUTHORIZATION_TOKEN= -e AWS_ROLE_ARN= -e AWS_SHARED_CREDENTIALS_FILE=/nonexistent-by-harness -e AWS_CONFIG_FILE=/nonexistent-by-harness -e AWS_WEB_IDENTITY_TOKEN_FILE=/nonexistent-by-harness rclone/rclone@sha256:c61954aaa32328a5486715dd063a81c7879f5195ad3505cd362deddd509dc4a1 lsjson --fast-list --files-only --use-server-modtime --no-mimetype -R :s3\,provider=AWS\,region=us-east-1:noaa-normals-pds
```
...
| | | |
| --- | --- | --- |
| `peak_rss` | 69.6 MB | `VmHWM` of the container's main process, 310 successful samples. **Main process only** — a multi-process fan-out mode's children are not included. |
| `cgroup_peak_mem` | 23.6 MB | cgroup v2 `memory.peak`, whole container tree, 310 successful samples. **Page cache and kernel/socket memory included. Never present this as RSS.** |
...
- stdout: external — `receipts/rclone/recursive-fastlist.noaa-normals-pds.full.anonymous.stdout.txt` (25846589 bytes, sha256 `ea5ce61ceec183417051c8d6ef9f389bae4e13e278a014c88d94cccf4faea679`) — redacted and scanned before hashing; published as a release asset at publication
- stderr: inline — `stderr.txt` (96 bytes, sha256 `7b590fd75e8843e3ecb8f7e5f6001da2d55db395702ed6060d2ed429785f1060`)
