...
| | |
| --- | --- |
| Date (UTC) | 2026-07-17T12:02:17Z |
| Exit code | `0` |
| Wall-clock | 1.744s (container lifetime, StartedAt→FinishedAt) |
...
```sh
docker run -d --network host -e AWS_EC2_METADATA_DISABLED=true -e TZ=UTC -e AWS_ACCESS_KEY_ID= -e AWS_SECRET_ACCESS_KEY= -e AWS_SESSION_TOKEN= -e AWS_SECURITY_TOKEN= -e AWS_CONTAINER_CREDENTIALS_RELATIVE_URI= -e AWS_CONTAINER_CREDENTIALS_FULL_URI= -e AWS_CONTAINER_AUTHORIZATION_TOKEN= -e AWS_ROLE_ARN= -e AWS_SHARED_CREDENTIALS_FILE=/nonexistent-by-harness -e AWS_CONFIG_FILE=/nonexistent-by-harness -e AWS_WEB_IDENTITY_TOKEN_FILE=/nonexistent-by-harness rclone/rclone@sha256:c61954aaa32328a5486715dd063a81c7879f5195ad3505cd362deddd509dc4a1 lsjson --fast-list --files-only --use-server-modtime --no-mimetype -R :s3\,provider=AWS\,region=us-east-1:noaa-normals-pds/normals-monthly/1991-2020/
```
...
| | | |
| --- | --- | --- |
| `peak_rss` | 67.4 MB | `VmHWM` of the container's main process, 31 successful samples. **Main process only** — a multi-process fan-out mode's children are not included. |
| `cgroup_peak_mem` | 22.2 MB | cgroup v2 `memory.peak`, whole container tree, 31 successful samples. **Page cache and kernel/socket memory included. Never present this as RSS.** |
...
- stdout: external — `receipts/rclone/recursive-fastlist.noaa-normals-pds.normals-monthly_1991-2020_.anonymous.stdout.txt` (2306279 bytes, sha256 `dbd58966ecb25b5b2ddac8118b1679ff4679f0f2d66f0378d1e4f7d9df3035e5`) — redacted and scanned before hashing; published as a release asset at publication
- stderr: inline — `stderr.txt` (96 bytes, sha256 `b6169dc20b1a8a6d260b62cf118eb3e4bc74cadc2ae05690717533f3c29cac54`)
