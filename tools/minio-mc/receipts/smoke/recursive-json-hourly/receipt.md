...
| | |
| --- | --- |
| Date (UTC) | 2026-07-17T12:27:57Z |
| Exit code | `0` |
| Wall-clock | 1.608s (container lifetime, StartedAt→FinishedAt) |
...
| | | |
| --- | --- | --- |
| `peak_rss` | 33.4 MB | `VmHWM` of the container's main process, 28 successful samples. **Main process only** — a multi-process fan-out mode's children are not included. |
| `cgroup_peak_mem` | 13.5 MB | cgroup v2 `memory.peak`, whole container tree, 28 successful samples. **Page cache and kernel/socket memory included. Never present this as RSS.** |
...
**Not exposed by mc as a built-in counter.** Request shape observed via `mc --debug` (curated trace: `receipts/smoke/_capability/debug-trace/`): one anonymous `GET /?location` (GetBucketLocation, resolves region), then **serial** `list-type=2 (recursive, delimiter empty)` requests, each page ≤1000 keys, advanced by the prior response's `continuation-token` (or version-id/key markers for the versions API) — inherently unparallelizable. Derived for this run: 2549 keys / 1000 → **~3 LIST page request(s)** plus the location probe. No `Authorization` header on any request (unsigned/anonymous). Counts are inferred from key volume + observed page size, not a tool-reported metric. [OBS]/[INFERRED]

## Raw output

- stdout: external — `receipts/minio-mc/recursive-json.noaa-normals-pds.normals-hourly_.anonymous.stdout.txt` (714712 bytes, sha256 `c4f066d71c5e355df0f4b21a9d1fdaccf1d836cec5ffd9c6a28565c7a73dc188`) — redacted and scanned before hashing; published as a release asset at publication
