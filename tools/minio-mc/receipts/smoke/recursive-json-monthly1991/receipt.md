...
| | |
| --- | --- |
| Date (UTC) | 2026-07-17T12:31:51Z |
| Exit code | `0` |
| Wall-clock | 9.740s (container lifetime, StartedAt→FinishedAt) |
...
| | | |
| --- | --- | --- |
| `peak_rss` | 34.8 MB | `VmHWM` of the container's main process, 178 successful samples. **Main process only** — a multi-process fan-out mode's children are not included. |
| `cgroup_peak_mem` | 15.9 MB | cgroup v2 `memory.peak`, whole container tree, 178 successful samples. **Page cache and kernel/socket memory included. Never present this as RSS.** |
...
**Not exposed by mc as a built-in counter.** Request shape observed via `mc --debug` (curated trace: `receipts/smoke/_capability/debug-trace/`): one anonymous `GET /?location` (GetBucketLocation, resolves region), then **serial** `list-type=2 (recursive, delimiter empty)` requests, each page ≤1000 keys, advanced by the prior response's `continuation-token` (or version-id/key markers for the versions API) — inherently unparallelizable. Derived for this run: 15625 keys / 1000 → **~16 LIST page request(s)** plus the location probe. No `Authorization` header on any request (unsigned/anonymous). Counts are inferred from key volume + observed page size, not a tool-reported metric. [OBS]/[INFERRED]

## Raw output

- stdout: external — `receipts/minio-mc/recursive-json.noaa-normals-pds.normals-monthly_1991-2020_.anonymous.stdout.txt` (4384074 bytes, sha256 `26ad2c41c447512589dea167d73916c7e42e2084550042338449d86320720b53`) — redacted and scanned before hashing; published as a release asset at publication
