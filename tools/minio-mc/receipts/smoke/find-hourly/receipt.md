# Smoke receipt — `minio-mc` / mode `find`
...
| | |
| --- | --- |
| Date (UTC) | 2026-07-17T12:44:55Z |
| Exit code | `0` |
| Wall-clock | 1.463s (container lifetime, StartedAt→FinishedAt) |
...
| | | |
| --- | --- | --- |
| `peak_rss` | 33.1 MB | `VmHWM` of the container's main process, 26 successful samples. **Main process only** — a multi-process fan-out mode's children are not included. |
| `cgroup_peak_mem` | 13.5 MB | cgroup v2 `memory.peak`, whole container tree, 26 successful samples. **Page cache and kernel/socket memory included. Never present this as RSS.** |
...
- stdout: external — `receipts/minio-mc/find.noaa-normals-pds.normals-hourly_.anonymous.stdout.txt` (169234 bytes, sha256 `ec6c54c8c22e2a202da994ad6d391345ef19a0d3d6de904c058221dbfc0d661a`) — redacted and scanned before hashing; published as a release asset at publication
