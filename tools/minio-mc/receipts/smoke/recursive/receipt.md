# Smoke receipt — `minio-mc` / mode `recursive`
...
| | |
| --- | --- |
| Date (UTC) | 2026-07-17T12:30:16Z |
| Exit code | `0` |
| Wall-clock | 75.381s (container lifetime, StartedAt→FinishedAt) |
...
| | | |
| --- | --- | --- |
| `peak_rss` | 34.5 MB | `VmHWM` of the container's main process, 1380 successful samples. **Main process only** — a multi-process fan-out mode's children are not included. |
| `cgroup_peak_mem` | 14.9 MB | cgroup v2 `memory.peak`, whole container tree, 1380 successful samples. **Page cache and kernel/socket memory included. Never present this as RSS.** |
...
- stdout: external — `receipts/minio-mc/recursive.noaa-normals-pds.full.anonymous.stdout.txt` (13495282 bytes, sha256 `f13c071690f78cd5bec34421382977b0f4607801f2e6a4280bff4ef1c8ddb856`) — redacted and scanned before hashing; published as a release asset at publication
