# Smoke receipt — `minio-mc` / mode `find-json`
...
| | |
| --- | --- |
| Date (UTC) | 2026-07-17T12:44:53Z |
| Exit code | `0` |
| Wall-clock | 1.623s (container lifetime, StartedAt→FinishedAt) |
...
| | | |
| --- | --- | --- |
| `peak_rss` | 33.4 MB | `VmHWM` of the container's main process, 29 successful samples. **Main process only** — a multi-process fan-out mode's children are not included. |
| `cgroup_peak_mem` | 13.4 MB | cgroup v2 `memory.peak`, whole container tree, 29 successful samples. **Page cache and kernel/socket memory included. Never present this as RSS.** |
...
**Not exposed by mc as a built-in counter.** `mc find` traverses via the same serial `ClientContent` List path as `mc ls --recursive` (`[SRC cmd/find.go:275-284 @ 7394ce0]`): one anonymous `GET /?location` then serial `list-type=2` pages (<=1000 keys each) advanced by `continuation-token`. Derived for this run: 2549 keys / 1000 -> **~3 LIST page request(s)** plus the location probe. No `Authorization` header (unsigned/anonymous). Inferred from key volume + observed page size, not a tool-reported metric. [OBS]/[INFERRED]

## Raw output

- stdout: external — `receipts/minio-mc/find-json.noaa-normals-pds.normals-hourly_.anonymous.stdout.txt` (429224 bytes, sha256 `301345fdf1af4eb9b7506b3c5a3195d2d9bae5c572ca2b2b04eec56c7ad20682`) — redacted and scanned before hashing; published as a release asset at publication
