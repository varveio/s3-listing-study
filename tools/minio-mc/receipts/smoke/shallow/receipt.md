# Smoke receipt — `minio-mc` / mode `shallow`
...
| | |
| --- | --- |
| Date (UTC) | 2026-07-17T12:27:44Z |
| Exit code | `0` |
| Wall-clock | 0.266s (container lifetime, StartedAt→FinishedAt) |
...
| | | |
| --- | --- | --- |
| `peak_rss` | 28.1 MB | `VmHWM` of the container's main process, 4 successful samples. **Main process only** — a multi-process fan-out mode's children are not included. |
| `cgroup_peak_mem` | 9.0 MB | cgroup v2 `memory.peak`, whole container tree, 4 successful samples. **Page cache and kernel/socket memory included. Never present this as RSS.** |
...
- stdout: inline — `stdout.txt` (257 bytes, sha256 `743733edef484d0d6cb284f554b0f773867b61e595d0ba82660cae198faf9f78`)
