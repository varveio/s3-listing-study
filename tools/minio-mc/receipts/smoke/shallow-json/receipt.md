# Smoke receipt — `minio-mc` / mode `shallow-json`
...
| | |
| --- | --- |
| Date (UTC) | 2026-07-17T12:27:45Z |
| Exit code | `0` |
| Wall-clock | 0.214s (container lifetime, StartedAt→FinishedAt) |
...
| | | |
| --- | --- | --- |
| `peak_rss` | 28.7 MB | `VmHWM` of the container's main process, 3 successful samples. **Main process only** — a multi-process fan-out mode's children are not included. |
| `cgroup_peak_mem` | 9.4 MB | cgroup v2 `memory.peak`, whole container tree, 3 successful samples. **Page cache and kernel/socket memory included. Never present this as RSS.** |
