# Smoke receipt — `minio-mc` / mode `versions-json`
...
| | |
| --- | --- |
| Date (UTC) | 2026-07-17T12:28:00Z |
| Exit code | `0` |
| Wall-clock | 1.711s (container lifetime, StartedAt→FinishedAt) |
...
| | | |
| --- | --- | --- |
| `peak_rss` | 31.9 MB | `VmHWM` of the container's main process, 30 successful samples. **Main process only** — a multi-process fan-out mode's children are not included. |
| `cgroup_peak_mem` | 12.5 MB | cgroup v2 `memory.peak`, whole container tree, 30 successful samples. **Page cache and kernel/socket memory included. Never present this as RSS.** |
...
**Not exposed by mc as a built-in counter.** This mode uses the **`GET ?versions`
(list-object-versions) API** [SRC cmd/client-s3.go:1904-1905,1566; minio-go
listObjectVersions], distinct from `list-type=2`; it paginates serially by
version-id/key markers. **Note:** the captured `--debug` trace
(`receipts/smoke/_capability/debug-trace/`) is for a **recursive `list-type=2`**
run, **not** this `?versions` run — it demonstrates the serial one-request-at-a-time
pagination shape and the absence of an `Authorization` header for the paginator
generally; the `?versions` request shape here is [INFERRED] from source (same
serial paginator), not read off that trace. Derived for this run: 2549 keys / 1000
→ **~3 page request(s)** plus an anonymous `GET /?location`. Counts inferred from
key volume, not a tool-reported metric. [INFERRED]

## Raw output

- stdout: external — `receipts/minio-mc/versions-json.noaa-normals-pds.normals-hourly_.anonymous.stdout.txt` (763143 bytes, sha256 `eea12791dd52bd1844c5b17c2c27e1ffffbc1637d8af69a71a5d1d4bc4c56d53`) — redacted and scanned before hashing; published as a release asset at publication
