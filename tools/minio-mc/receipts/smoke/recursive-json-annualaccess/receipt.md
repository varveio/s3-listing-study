...
| | |
| --- | --- |
| Date (UTC) | 2026-07-17T12:32:02Z |
| Exit code | `0` |
| Wall-clock | 4.708s (container lifetime, StartedAt→FinishedAt) |
...
| | | |
| --- | --- | --- |
| `peak_rss` | 33.6 MB | `VmHWM` of the container's main process, 85 successful samples. **Main process only** — a multi-process fan-out mode's children are not included. |
| `cgroup_peak_mem` | 14.8 MB | cgroup v2 `memory.peak`, whole container tree, 85 successful samples. **Page cache and kernel/socket memory included. Never present this as RSS.** |
...
**Not exposed by mc as a built-in counter.** Request shape observed via `mc --debug` (curated trace: `receipts/smoke/_capability/debug-trace/`): one anonymous `GET /?location` (GetBucketLocation, resolves region), then **serial** `list-type=2 (recursive, delimiter empty)` requests, each page ≤1000 keys, advanced by the prior response's `continuation-token` (or version-id/key markers for the versions API) — inherently unparallelizable. Derived for this run: 9839 keys / 1000 → **~10 LIST page request(s)** plus the location probe. No `Authorization` header on any request (unsigned/anonymous). Counts are inferred from key volume + observed page size, not a tool-reported metric. [OBS]/[INFERRED]

## Raw output

- stdout: external — `receipts/minio-mc/recursive-json.noaa-normals-pds.normals-annualseasonal_1981-2010_access_.anonymous.stdout.txt` (2830841 bytes, sha256 `9ca2e239c7dde6448bf14198e6f3065f8cc3354f4d21052bea7a68d42f1770ee`) — redacted and scanned before hashing; published as a release asset at publication
