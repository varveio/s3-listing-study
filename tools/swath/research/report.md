...
derivation this update re-tested; that layer is retired and reachable at
commit `8733e22` (see [`README.md`](README.md) § Retired).
...
readers' change-driven sweep of their areas (the added claims below), not by a
blind read. Where the two methods would have differed is in what a fresh reader might
...
The seven v0.2.0 run-observation claims listed as `observation` were dropped when that layer was retired; their rows are kept so the re-test record is complete.
