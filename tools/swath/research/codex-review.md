...
8. **MEDIUM — Aligned normalization assumes a timestamp width the source does not guarantee.**  
   [report.md:193](report.md), [report.md:204](report.md), `normalize.sh:50`, `S3PageFetcher.java:724`, `Fields.java:13`. Source preserves microseconds and `ISO_INSTANT` may emit more than 24 characters; the aligned formatter treats 24 as a minimum, while the normalizer unconditionally takes 24 characters and starts the key at column 43. Fractional microseconds therefore truncate the timestamp and shift bytes into the key. Its claim to drop CommonPrefix/DeleteMarker rows is also false for aligned output, which exposes no row type.
