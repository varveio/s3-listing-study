...
| | |
| --- | --- |
| Date (UTC) | 2026-07-17T12:57:12Z |
| Exit code | `0` |
| Wall-clock | 7.535s (container lifetime, StartedAt→FinishedAt) |
...
| | | |
| --- | --- | --- |
| `peak_rss` | 405.0 MB | `VmHWM` of the container's main process, 135 successful samples. **Main process only** — a multi-process fan-out mode's children are not included. |
| `cgroup_peak_mem` | 385.9 MB | cgroup v2 `memory.peak`, whole container tree, 136 successful samples. **Page cache and kernel/socket memory included. Never present this as RSS.** |
...
- `api_calls` (total ListObjectsV2 issued): **319**
- `api_calls_per_1k_objects`: 2.1421328659588896
- worker range pages: 173; `peak_in_flight` (max concurrent LISTs): 8
- `probe_fetches` (speculative 1-key pivot + delimiter=/ structure probes): 70; `empty_upper_bisections`: 39
- `steals`=102 `splits`=8 `errors`=0 `throttle_events`=0 `transient_events`=0
...
- stdout: external — `receipts/swath/recursive-jsonl.noaa-normals-pds.full.anonymous.stdout.txt` (38502008 bytes, sha256 `0cb1faf9896cd7856fabc970dc185e70865f6ac9d8edd6c800be72fde5b8e1ac`) — redacted and scanned before hashing; published as a release asset at publication
- stderr: inline — `stderr.txt` (3333 bytes, sha256 `9c4ef6308b7c531a30373cadadb217ec70c7ab9d39bbc7ff05ead356fb26c7e6`)
