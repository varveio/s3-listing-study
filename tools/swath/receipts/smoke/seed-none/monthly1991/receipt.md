...
| | |
| --- | --- |
| Date (UTC) | 2026-07-17T12:57:51Z |
| Exit code | `0` |
| Wall-clock | 3.649s (container lifetime, StartedAt→FinishedAt) |
...
| | | |
| --- | --- | --- |
| `peak_rss` | 354.9 MB | `VmHWM` of the container's main process, 65 successful samples. **Main process only** — a multi-process fan-out mode's children are not included. |
| `cgroup_peak_mem` | 335.7 MB | cgroup v2 `memory.peak`, whole container tree, 65 successful samples. **Page cache and kernel/socket memory included. Never present this as RSS.** |
...
- `api_calls` (total ListObjectsV2 issued): **116**
- `api_calls_per_1k_objects`: 7.424
- worker range pages: 20; `peak_in_flight` (max concurrent LISTs): 4
- `probe_fetches` (speculative 1-key pivot + delimiter=/ structure probes): 82; `empty_upper_bisections`: 47
- `steals`=80 `splits`=7 `errors`=0 `throttle_events`=0 `transient_events`=0
...
- stdout: external — `receipts/swath/seed-none.noaa-normals-pds.normals-monthly_1991-2020_.anonymous.stdout.txt` (1946625 bytes, sha256 `e5dcf02f04b1a05b4b1569188ef7c9b8ef9e7194debf6d6132339a39a15326af`) — redacted and scanned before hashing; published as a release asset at publication
- stderr: inline — `stderr.txt` (2891 bytes, sha256 `b79674b8e48e31dd345ede95e47aa7aedcf5ca033b5523d0598fa9274572e085`)
