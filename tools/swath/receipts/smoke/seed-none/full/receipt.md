# Smoke receipt — `swath` / mode `seed-none`
...
| | |
| --- | --- |
| Date (UTC) | 2026-07-17T12:57:41Z |
| Exit code | `0` |
| Wall-clock | 9.161s (container lifetime, StartedAt→FinishedAt) |
...
| | | |
| --- | --- | --- |
| `peak_rss` | 400.3 MB | `VmHWM` of the container's main process, 165 successful samples. **Main process only** — a multi-process fan-out mode's children are not included. |
| `cgroup_peak_mem` | 381.2 MB | cgroup v2 `memory.peak`, whole container tree, 165 successful samples. **Page cache and kernel/socket memory included. Never present this as RSS.** |
...
- `api_calls` (total ListObjectsV2 issued): **516**
- `api_calls_per_1k_objects`: 3.4650174258143798
- worker range pages: 159; `peak_in_flight` (max concurrent LISTs): 6
- `probe_fetches` (speculative 1-key pivot + delimiter=/ structure probes): 291; `empty_upper_bisections`: 194
- `steals`=284 `splits`=22 `errors`=0 `throttle_events`=0 `transient_events`=0
...
- stdout: external — `receipts/swath/seed-none.noaa-normals-pds.full.anonymous.stdout.txt` (18547181 bytes, sha256 `582ed0cc9a8c7ad0aaa4922e2b4e614329831fb8eb2388771e068784ac9ce22e`) — redacted and scanned before hashing; published as a release asset at publication
- stderr: inline — `stderr.txt` (3202 bytes, sha256 `25d2801d7e8ee1e2efe15e5c1b02adb7e90873ebbb654515e61469fd9284cb97`)
