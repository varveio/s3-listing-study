...
| | |
| --- | --- |
| Date (UTC) | 2026-07-17T12:57:03Z |
| Exit code | `0` |
| Wall-clock | 3.825s (container lifetime, StartedAt→FinishedAt) |
...
| | | |
| --- | --- | --- |
| `peak_rss` | 364.2 MB | `VmHWM` of the container's main process, 68 successful samples. **Main process only** — a multi-process fan-out mode's children are not included. |
| `cgroup_peak_mem` | 344.6 MB | cgroup v2 `memory.peak`, whole container tree, 68 successful samples. **Page cache and kernel/socket memory included. Never present this as RSS.** |
...
- `api_calls` (total ListObjectsV2 issued): **123**
- `api_calls_per_1k_objects`: 7.872
...
- `steals`=59 `splits`=7 `errors`=0 `throttle_events`=0 `transient_events`=0
...
- stdout: external — `receipts/swath/recursive-tsv.noaa-normals-pds.normals-monthly_1991-2020_.anonymous.stdout.txt` (1946625 bytes, sha256 `eef8b28d18547830566e11fcb993ca71a13887bd36a2b274566de33e008f9d63`) — redacted and scanned before hashing; published as a release asset at publication
- stderr: inline — `stderr.txt` (3271 bytes, sha256 `5348473b7203201bf0eba1f7075084701093c13c8aeaa354508f3183bd7890b0`)
