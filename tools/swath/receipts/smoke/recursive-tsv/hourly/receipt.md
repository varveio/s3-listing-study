# Smoke receipt — `swath` / mode `recursive-tsv`
...
| | |
| --- | --- |
| Date (UTC) | 2026-07-17T12:54:36Z |
| Exit code | `0` |
| Wall-clock | 3.985s (container lifetime, StartedAt→FinishedAt) |
...
| | | |
| --- | --- | --- |
| `peak_rss` | 381.8 MB | `VmHWM` of the container's main process, 71 successful samples. **Main process only** — a multi-process fan-out mode's children are not included. |
| `cgroup_peak_mem` | 372.2 MB | cgroup v2 `memory.peak`, whole container tree, 71 successful samples. **Page cache and kernel/socket memory included. Never present this as RSS.** |
...
- `api_calls` (total ListObjectsV2 issued): **112**
- `api_calls_per_1k_objects`: 43.93879952922715
- worker range pages: 18; `peak_in_flight` (max concurrent LISTs): 8
- `probe_fetches` (speculative 1-key pivot + delimiter=/ structure probes): 45; `empty_upper_bisections`: 40
- `steals`=6 `splits`=0 `errors`=0 `throttle_events`=0 `transient_events`=0
...
- stdout: external — `receipts/swath/recursive-tsv.noaa-normals-pds.normals-hourly_.anonymous.stdout.txt` (317119 bytes, sha256 `91865f49aa877e4833c3138e7be0e2cc9d40e42533668fae163c64b22f65bf05`) — redacted and scanned before hashing; published as a release asset at publication
- stderr: inline — `stderr.txt` (2887 bytes, sha256 `c75cf931ff5cdb71a02cd9928f897ff6917d7b6e16a44d82c80fce172b7aca69`)
