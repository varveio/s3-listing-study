...
| | |
| --- | --- |
| Date (UTC) | 2026-07-17T12:56:18Z |
| Exit code | `0` |
| Wall-clock | 7.171s (container lifetime, StartedAt→FinishedAt) |
...
| | | |
| --- | --- | --- |
| `peak_rss` | 531.9 MB | `VmHWM` of the container's main process, 128 successful samples. **Main process only** — a multi-process fan-out mode's children are not included. |
| `cgroup_peak_mem` | 513.0 MB | cgroup v2 `memory.peak`, whole container tree, 129 successful samples. **Page cache and kernel/socket memory included. Never present this as RSS.** |
...
- `api_calls` (total ListObjectsV2 issued): **339**
- `api_calls_per_1k_objects`: 2.276435866959447
- worker range pages: 172; `peak_in_flight` (max concurrent LISTs): 8
- `probe_fetches` (speculative 1-key pivot + delimiter=/ structure probes): 97; `empty_upper_bisections`: 60
- `steals`=98 `splits`=7 `errors`=0 `throttle_events`=0 `transient_events`=0
...
- stdout: external — `receipts/swath/recursive-tsv.noaa-normals-pds.full.anonymous.stdout.txt` (18547181 bytes, sha256 `81c3f733363e0076cdc4c8f9401daf79651e66f3380270fbe7e408c45ecdf475`) — redacted and scanned before hashing; published as a release asset at publication
- stderr: inline — `stderr.txt` (3367 bytes, sha256 `e1d6e0dddc7900787b7f808b31b8d3f3e3462fa5beff9039d17348a9a0325158`)
