...
| | |
| --- | --- |
| Date (UTC) | 2026-07-17T12:57:36Z |
| Exit code | `0` |
| Wall-clock | 3.865s (container lifetime, StartedAt→FinishedAt) |
...
| | | |
| --- | --- | --- |
| `peak_rss` | 380.1 MB | `VmHWM` of the container's main process, 69 successful samples. **Main process only** — a multi-process fan-out mode's children are not included. |
| `cgroup_peak_mem` | 360.7 MB | cgroup v2 `memory.peak`, whole container tree, 69 successful samples. **Page cache and kernel/socket memory included. Never present this as RSS.** |
...
- `api_calls` (total ListObjectsV2 issued): **119**
- `api_calls_per_1k_objects`: 7.616
- worker range pages: 28; `peak_in_flight` (max concurrent LISTs): 8
- `probe_fetches` (speculative 1-key pivot + delimiter=/ structure probes): 43; `empty_upper_bisections`: 23
- `steals`=56 `splits`=7 `errors`=0 `throttle_events`=0 `transient_events`=0
...
- stdout: external — `receipts/swath/recursive-aligned.noaa-normals-pds.normals-monthly_1991-2020_.anonymous.stdout.txt` (1422187 bytes, sha256 `a15d629a0176e871503abc58505b7933d083621032762a4e4b1b8e8827c85cc5`) — redacted and scanned before hashing; published as a release asset at publication
- stderr: inline — `stderr.txt` (3287 bytes, sha256 `e71b7056960948ffd819b0a1765773a2593238de0be643c2d39fd682cc2d608b`)
