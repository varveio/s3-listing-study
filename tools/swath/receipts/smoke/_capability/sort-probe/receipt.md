# Smoke receipt — `swath` / mode `sort-probe`
...
| | |
| --- | --- |
| Date (UTC) | 2026-07-17T12:59:34Z |
| Exit code | `0` |
| Wall-clock | 4.822s (container lifetime, StartedAt→FinishedAt) |
...
| | | |
| --- | --- | --- |
| `peak_rss` | 402.6 MB | `VmHWM` of the container's main process, 86 successful samples. **Main process only** — a multi-process fan-out mode's children are not included. |
| `cgroup_peak_mem` | 385.1 MB | cgroup v2 `memory.peak`, whole container tree, 87 successful samples. **Page cache and kernel/socket memory included. Never present this as RSS.** |
...
- `api_calls` (total ListObjectsV2 issued): **135**
- `api_calls_per_1k_objects`: 8.64
- worker range pages: 28; `peak_in_flight` (max concurrent LISTs): 8
- `probe_fetches` (speculative 1-key pivot + delimiter=/ structure probes): 58; `empty_upper_bisections`: 37
- `steals`=63 `splits`=7 `errors`=0 `throttle_events`=0 `transient_events`=0
...
- stderr: inline — `stderr.txt` (4000 bytes, sha256 `b60ed54f03f9e414a21051c9063f98692230029a7ff9bd1480dee26ba2139ce6`)
