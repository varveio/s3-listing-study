# Smoke receipt — `s5cmd` / mode `fullpath`
...
| | |
| --- | --- |
| Date (UTC) | 2026-07-17T09:00:35Z |
| Exit code | `0` |
| Wall-clock | 17.099s (container lifetime, StartedAt→FinishedAt) |
...
| | | |
| --- | --- | --- |
| `peak_rss` | 40.6 MB | `VmHWM` of the container's main process, 315 successful samples. **Main process only** — a multi-process fan-out mode's children are not included. |
| `cgroup_peak_mem` | 30.7 MB | cgroup v2 `memory.peak`, whole container tree, 315 successful samples. **Page cache and kernel/socket memory included. Never present this as RSS.** |
...
**Obtainable via `--log trace`** (s5cmd surfaces no built-in numeric counter); see `receipts/smoke/_capability/observability/` [OBS]. This wrapper run did not enable trace.

## Raw output

- stdout: external — `receipts/s5cmd/fullpath.noaa-normals-pds.full.anonymous.stdout.txt` (10516942 bytes, sha256 `cf85550f3fa6faf6db1266ae8aeea9a760cb9bae2893ebaf2154dc0cccd90047`) — redacted and scanned before hashing; published as a release asset at publication
