# Smoke receipt — `s5cmd` / mode `allversions`
...
| | |
| --- | --- |
| Date (UTC) | 2026-07-17T08:14:29Z |
| Exit code | `0` |
| Wall-clock | 86.990s (container lifetime, StartedAt→FinishedAt) |
...
| | | |
| --- | --- | --- |
| `peak_rss` | 52.6 MB | `VmHWM` of the container's main process, 1604 successful samples. **Main process only** — a multi-process fan-out mode's children are not included. |
| `cgroup_peak_mem` | 43.0 MB | cgroup v2 `memory.peak`, whole container tree, 1604 successful samples. **Page cache and kernel/socket memory included. Never present this as RSS.** |
...
- stdout: external — `receipts/s5cmd/allversions.noaa-normals-pds.full.anonymous.stdout.txt` (20747357 bytes, sha256 `767b7272ad17ec4482499aff2ced0552049535d5532e53c0f07f3ef57ffb9b06`) — redacted and scanned before hashing; published as a release asset at publication
