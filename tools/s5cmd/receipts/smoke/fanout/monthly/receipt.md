...
| | |
| --- | --- |
| Date (UTC) | 2026-07-17T08:28:50Z |
| Exit code | `0` |
| Wall-clock | 4.816s (container lifetime, StartedAt→FinishedAt) |
...
| | | |
| --- | --- | --- |
| `peak_rss` | 39.1 MB | `VmHWM` of the container's main process, 88 successful samples. **Main process only** — a multi-process fan-out mode's children are not included. |
| `cgroup_peak_mem` | 29.3 MB | cgroup v2 `memory.peak`, whole container tree, 88 successful samples. **Page cache and kernel/socket memory included. Never present this as RSS.** |
...
- stdout: external — `receipts/s5cmd/recursive.noaa-normals-pds.normals-monthly_.anonymous.stdout.txt` (5513918 bytes, sha256 `2d35c7a383d0f5dc1037ffd556de0be1620f0798b4ca6678a69faee0e0a50323`) — redacted and scanned before hashing; published as a release asset at publication
