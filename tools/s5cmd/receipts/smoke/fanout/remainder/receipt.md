# Smoke receipt — `s5cmd` / mode `rootkeys`
...
| | |
| --- | --- |
| Date (UTC) | 2026-07-17T08:29:08Z |
| Exit code | `0` |
| Wall-clock | 0.158s (container lifetime, StartedAt→FinishedAt) |
...
| Passed env (--env) | none |
| Verifier verdict | **PASS (as the union remainder)** — this shard carries the *orphan* (unprefixed) keys and has no single-receipt scope; its verdict is in `../union/union-verify.md` (`--scope union`, Verdict PASS), which re-derives and checks it against the manifest's unprefixed keys. Not independently stampable by the single-receipt path. |
| Tool version | `v2.3.0-991c9fb` — caller-supplied |

## Invocation

```sh
docker run -d --network host -e AWS_EC2_METADATA_DISABLED=true -e TZ=UTC -e AWS_ACCESS_KEY_ID= -e AWS_SECRET_ACCESS_KEY= -e AWS_SESSION_TOKEN= -e AWS_SECURITY_TOKEN= -e AWS_CONTAINER_CREDENTIALS_RELATIVE_URI= -e AWS_CONTAINER_CREDENTIALS_FULL_URI= -e AWS_CONTAINER_AUTHORIZATION_TOKEN= -e AWS_ROLE_ARN= -e AWS_SHARED_CREDENTIALS_FILE=/nonexistent-by-harness -e AWS_CONFIG_FILE=/nonexistent-by-harness -e AWS_WEB_IDENTITY_TOKEN_FILE=/nonexistent-by-harness peakcom/s5cmd@sha256:2ff939e2ee3c76adcadd78dbfc3e2569b18a3743ed9dcfccb1ec589af7fb9903 --no-sign-request ls -e -s s3://noaa-normals-pds/
```
...
| `cgroup_peak_mem` | 9.6 MB | cgroup v2 `memory.peak`, whole container tree, 2 successful samples. **Page cache and kernel/socket memory included. Never present this as RSS.** |
...
- stdout: inline — `stdout.txt` (469 bytes, sha256 `98eb4af2065d40baf43852c2e442047d91a108d0df111d98634f38e5a0c6c9a6`)
