...
| | |
| --- | --- |
| Date (UTC) | 2026-07-17T08:29:01Z |
| Exit code | `0` |
| Wall-clock | 4.996s (container lifetime, StartedAt→FinishedAt) |
...
| | | |
| --- | --- | --- |
| `peak_rss` | 39.1 MB | `VmHWM` of the container's main process, 91 successful samples. **Main process only** — a multi-process fan-out mode's children are not included. |
| `cgroup_peak_mem` | 29.6 MB | cgroup v2 `memory.peak`, whole container tree, 91 successful samples. **Page cache and kernel/socket memory included. Never present this as RSS.** |
...
- stdout: external — `receipts/s5cmd/recursive.noaa-normals-pds.normals-annualseasonal_.anonymous.stdout.txt` (5512154 bytes, sha256 `ec9527a4ccfb96aa8e9651647596bb919498fa2262cb3ebe12cc76a1677153dd`) — redacted and scanned before hashing; published as a release asset at publication
