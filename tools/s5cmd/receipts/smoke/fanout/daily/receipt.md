...
| | |
| --- | --- |
| Date (UTC) | 2026-07-17T08:28:55Z |
| Exit code | `0` |
| Wall-clock | 5.576s (container lifetime, StartedAt→FinishedAt) |
...
| | | |
| --- | --- | --- |
| `peak_rss` | 40.4 MB | `VmHWM` of the container's main process, 102 successful samples. **Main process only** — a multi-process fan-out mode's children are not included. |
| `cgroup_peak_mem` | 30.7 MB | cgroup v2 `memory.peak`, whole container tree, 102 successful samples. **Page cache and kernel/socket memory included. Never present this as RSS.** |
...
- stdout: external — `receipts/s5cmd/recursive.noaa-normals-pds.normals-daily_.anonymous.stdout.txt` (5512739 bytes, sha256 `6f8e4f897a0a96adb2fb8ff76320f862349ecce2b0f3f0b4084f70d46826c506`) — redacted and scanned before hashing; published as a release asset at publication
