...
| | |
| --- | --- |
| Date (UTC) | 2026-07-17T08:11:57Z |
| Exit code | `0` |
| Wall-clock | 0.542s (container lifetime, StartedAt→FinishedAt) |
...
| | | |
| --- | --- | --- |
| `peak_rss` | 27.2 MB | `VmHWM` of the container's main process, 9 successful samples. **Main process only** — a multi-process fan-out mode's children are not included. |
| `cgroup_peak_mem` | 17.1 MB | cgroup v2 `memory.peak`, whole container tree, 9 successful samples. **Page cache and kernel/socket memory included. Never present this as RSS.** |
