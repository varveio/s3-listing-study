...
| | |
| --- | --- |
| Date (UTC) | 2026-07-17T08:12:03Z |
| Exit code | `0` |
| Wall-clock | 15.774s (container lifetime, StartedAt→FinishedAt) |
...
| | | |
| --- | --- | --- |
| `peak_rss` | 39.6 MB | `VmHWM` of the container's main process, 291 successful samples. **Main process only** — a multi-process fan-out mode's children are not included. |
| `cgroup_peak_mem` | 29.9 MB | cgroup v2 `memory.peak`, whole container tree, 291 successful samples. **Page cache and kernel/socket memory included. Never present this as RSS.** |
...
- stdout: external — `receipts/s5cmd/json.noaa-normals-pds.full.anonymous.stdout.txt` (32098577 bytes, sha256 `a231ef88f1e99eb6d2efb24ef80b069d03d4eece24f5eb4088890bf2b743be72`) — redacted and scanned before hashing; published as a release asset at publication
