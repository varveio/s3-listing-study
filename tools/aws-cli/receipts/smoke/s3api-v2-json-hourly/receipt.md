# Smoke receipt — `aws-cli` / mode `s3api-v2-json`
...
| | |
| --- | --- |
| Date (UTC) | 2026-07-17T08:15:32Z |
| Exit code | `0` |
| Wall-clock | 1.159s (container lifetime, StartedAt→FinishedAt) |
...
```sh
docker run -d --network host -e AWS_EC2_METADATA_DISABLED=true -e TZ=UTC -e AWS_ACCESS_KEY_ID= -e AWS_SECRET_ACCESS_KEY= -e AWS_SESSION_TOKEN= -e AWS_SECURITY_TOKEN= -e AWS_CONTAINER_CREDENTIALS_RELATIVE_URI= -e AWS_CONTAINER_CREDENTIALS_FULL_URI= -e AWS_CONTAINER_AUTHORIZATION_TOKEN= -e AWS_ROLE_ARN= -e AWS_SHARED_CREDENTIALS_FILE=/nonexistent-by-harness -e AWS_CONFIG_FILE=/nonexistent-by-harness -e AWS_WEB_IDENTITY_TOKEN_FILE=/nonexistent-by-harness amazon/aws-cli@sha256:406ca32d31e640a56e8d52921b40528cc64bfa59ec9cb4ee1456db6746cb7292 s3api list-objects-v2 --bucket noaa-normals-pds --region us-east-1 --no-sign-request --prefix normals-hourly/ --output json
```
...
| | | |
| --- | --- | --- |
| `peak_rss` | 79.5 MB | `VmHWM` of the container's main process, 20 successful samples. **Main process only** — a multi-process fan-out mode's children are not included. |
| `cgroup_peak_mem` | 64.7 MB | cgroup v2 `memory.peak`, whole container tree, 20 successful samples. **Page cache and kernel/socket memory included. Never present this as RSS.** |
...
- stdout: external — `receipts/aws-cli/s3api-v2-json.noaa-normals-pds.normals-hourly_.anonymous.stdout.txt` (997739 bytes, sha256 `79b6e746930b569c600f23c7ef1ccaec6ee8523c9a558dcc56a616e80eec1385`) — redacted and scanned before hashing; published as a release asset at publication
