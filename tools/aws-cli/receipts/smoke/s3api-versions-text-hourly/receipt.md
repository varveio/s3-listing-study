# Smoke receipt — `aws-cli` / mode `s3api-versions-text`
...
| | |
| --- | --- |
| Date (UTC) | 2026-07-17T08:15:38Z |
| Exit code | `0` |
| Wall-clock | 2.204s (container lifetime, StartedAt→FinishedAt) |
...
```sh
docker run -d --network host -e AWS_EC2_METADATA_DISABLED=true -e TZ=UTC -e AWS_ACCESS_KEY_ID= -e AWS_SECRET_ACCESS_KEY= -e AWS_SESSION_TOKEN= -e AWS_SECURITY_TOKEN= -e AWS_CONTAINER_CREDENTIALS_RELATIVE_URI= -e AWS_CONTAINER_CREDENTIALS_FULL_URI= -e AWS_CONTAINER_AUTHORIZATION_TOKEN= -e AWS_ROLE_ARN= -e AWS_SHARED_CREDENTIALS_FILE=/nonexistent-by-harness -e AWS_CONFIG_FILE=/nonexistent-by-harness -e AWS_WEB_IDENTITY_TOKEN_FILE=/nonexistent-by-harness amazon/aws-cli@sha256:406ca32d31e640a56e8d52921b40528cc64bfa59ec9cb4ee1456db6746cb7292 s3api list-object-versions --bucket noaa-normals-pds --region us-east-1 --no-sign-request --prefix normals-hourly/ --query Versions\[\].\[Key\,Size\,ETag\,LastModified\,StorageClass\] --output text
```
...
| | | |
| --- | --- | --- |
| `peak_rss` | 80.8 MB | `VmHWM` of the container's main process, 40 successful samples. **Main process only** — a multi-process fan-out mode's children are not included. |
| `cgroup_peak_mem` | 66.4 MB | cgroup v2 `memory.peak`, whole container tree, 40 successful samples. **Page cache and kernel/socket memory included. Never present this as RSS.** |
...
- LIST requests (derived): **3** — 2,549 versions (ListObjectVersions, KeyMarker pagination)
- Evidence: [INFERRED from ceil + [OBS --debug] `_capability/README.md`]

## Raw output

- stdout: external — `receipts/aws-cli/s3api-versions-text.noaa-normals-pds.normals-hourly_.anonymous.stdout.txt` (317068 bytes, sha256 `e9f66340ff17d3771a485a099dd7c79aefaeb910bd9741a07e4f440d9fd6c197`) — redacted and scanned before hashing; published as a release asset at publication
