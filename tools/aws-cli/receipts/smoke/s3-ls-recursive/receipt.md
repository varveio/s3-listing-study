# Smoke receipt — `aws-cli` / mode `s3-ls-recursive`
...
| | |
| --- | --- |
| Date (UTC) | 2026-07-17T08:18:02Z |
| Exit code | `0` |
| Wall-clock | 27.035s (container lifetime, StartedAt→FinishedAt) |
...
| | | |
| --- | --- | --- |
| `peak_rss` | 80.9 MB | `VmHWM` of the container's main process, 499 successful samples. **Main process only** — a multi-process fan-out mode's children are not included. |
| `cgroup_peak_mem` | 67.0 MB | cgroup v2 `memory.peak`, whole container tree, 499 successful samples. **Page cache and kernel/socket memory included. Never present this as RSS.** |
...
- stdout: external — `receipts/aws-cli/s3-ls-recursive.noaa-normals-pds.full.anonymous.stdout.txt` (11857195 bytes, sha256 `0995bdc04fb83e4289e989894208af3cb002610905acb93a219dab9a312639f2`) — redacted and scanned before hashing; published as a release asset at publication
...
**PASS** — `harness/verify-listing.sh` scope `full`, expected=148917, distinct=148917, dups=0, missing=0, extra=0, field-mismatches=0. Verified against manifest sha256 `c78a82737dd1982a999912afa89f870c013cb22e01e50b8c4835ddb725992adb` (snapshot 2026-07-17). Adapter: `tools/aws-cli/normalize.sh` mode `s3-ls-recursive`.
