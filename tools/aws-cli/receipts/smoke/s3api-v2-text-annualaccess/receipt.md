...
| | |
| --- | --- |
| Date (UTC) | 2026-07-17T08:14:58Z |
| Exit code | `0` |
| Wall-clock | 2.124s (container lifetime, StartedAt→FinishedAt) |
...
| | | |
| --- | --- | --- |
| `peak_rss` | 79.9 MB | `VmHWM` of the container's main process, 38 successful samples. **Main process only** — a multi-process fan-out mode's children are not included. |
| `cgroup_peak_mem` | 65.9 MB | cgroup v2 `memory.peak`, whole container tree, 38 successful samples. **Page cache and kernel/socket memory included. Never present this as RSS.** |
...
- LIST requests (derived): **10** — 9,839 objects
- Evidence: [INFERRED from ceil + [OBS --debug] `_capability/README.md`]

## Raw output

- stdout: external — `receipts/aws-cli/s3api-v2-text.noaa-normals-pds.normals-annualseasonal_1981-2010_access_.anonymous.stdout.txt` (1295957 bytes, sha256 `577007277b8430f5c52ee8ec71ead9215ff8596db74951d66350a6c56fd2101d`) — redacted and scanned before hashing; published as a release asset at publication
