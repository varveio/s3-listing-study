...
| | |
| --- | --- |
| Date (UTC) | 2026-07-17T08:19:20Z |
| Exit code | `0` |
| Wall-clock | 7.606s (container lifetime, StartedAt→FinishedAt) |
...
```sh
docker run -d --network host -e AWS_EC2_METADATA_DISABLED=true -e TZ=UTC -e AWS_ACCESS_KEY_ID= -e AWS_SECRET_ACCESS_KEY= -e AWS_SESSION_TOKEN= -e AWS_SECURITY_TOKEN= -e AWS_CONTAINER_CREDENTIALS_RELATIVE_URI= -e AWS_CONTAINER_CREDENTIALS_FULL_URI= -e AWS_CONTAINER_AUTHORIZATION_TOKEN= -e AWS_ROLE_ARN= -e AWS_SHARED_CREDENTIALS_FILE=/nonexistent-by-harness -e AWS_CONFIG_FILE=/nonexistent-by-harness -e AWS_WEB_IDENTITY_TOKEN_FILE=/nonexistent-by-harness amazon/aws-cli@sha256:406ca32d31e640a56e8d52921b40528cc64bfa59ec9cb4ee1456db6746cb7292 s3api list-objects-v2 --bucket noaa-normals-pds --region us-east-1 --no-sign-request --prefix normals-daily/ --query Contents\[\].\[Key\,Size\,ETag\,LastModified\,StorageClass\] --output text
```
...
| Region | `us-east-1` |
| Prefix scope | `normals-daily/`normals-daily/ |
...
| | | |
| --- | --- | --- |
| `peak_rss` | 80.2 MB | `VmHWM` of the container's main process, 140 successful samples. **Main process only** — a multi-process fan-out mode's children are not included. |
| `cgroup_peak_mem` | 65.9 MB | cgroup v2 `memory.peak`, whole container tree, 140 successful samples. **Page cache and kernel/socket memory included. Never present this as RSS.** |
...
- LIST requests (derived): **49** — 48,787 objects
- Evidence: [INFERRED from ceil + [OBS --debug] `_capability/README.md`]

## Raw output

- stdout: external — `receipts/aws-cli/s3api-v2-text.noaa-normals-pds.normals-daily_.anonymous.stdout.txt` (5938376 bytes, sha256 `90bd2bb7d499a6b6f8ba5677aa39ceb9de34413259714bf5d5d74136236f349f`) — redacted and scanned before hashing; published as a release asset at publication
