...
| | |
| --- | --- |
| Date (UTC) | 2026-07-17T08:19:11Z |
| Exit code | `0` |
| Wall-clock | 8.453s (container lifetime, StartedAt→FinishedAt) |
...
```sh
docker run -d --network host -e AWS_EC2_METADATA_DISABLED=true -e TZ=UTC -e AWS_ACCESS_KEY_ID= -e AWS_SECRET_ACCESS_KEY= -e AWS_SESSION_TOKEN= -e AWS_SECURITY_TOKEN= -e AWS_CONTAINER_CREDENTIALS_RELATIVE_URI= -e AWS_CONTAINER_CREDENTIALS_FULL_URI= -e AWS_CONTAINER_AUTHORIZATION_TOKEN= -e AWS_ROLE_ARN= -e AWS_SHARED_CREDENTIALS_FILE=/nonexistent-by-harness -e AWS_CONFIG_FILE=/nonexistent-by-harness -e AWS_WEB_IDENTITY_TOKEN_FILE=/nonexistent-by-harness amazon/aws-cli@sha256:406ca32d31e640a56e8d52921b40528cc64bfa59ec9cb4ee1456db6746cb7292 s3api list-objects-v2 --bucket noaa-normals-pds --region us-east-1 --no-sign-request --prefix normals-monthly/ --query Contents\[\].\[Key\,Size\,ETag\,LastModified\,StorageClass\] --output text
```
...
| Region | `us-east-1` |
| Prefix scope | `normals-monthly/`normals-monthly/ |
...
| | | |
| --- | --- | --- |
| `peak_rss` | 79.9 MB | `VmHWM` of the container's main process, 156 successful samples. **Main process only** — a multi-process fan-out mode's children are not included. |
| `cgroup_peak_mem` | 66.0 MB | cgroup v2 `memory.peak`, whole container tree, 156 successful samples. **Page cache and kernel/socket memory included. Never present this as RSS.** |
...
- LIST requests (derived): **49** — 48,796 objects
- Evidence: [INFERRED from ceil + [OBS --debug] `_capability/README.md`]

## Raw output

- stdout: external — `receipts/aws-cli/s3api-v2-text.noaa-normals-pds.normals-monthly_.anonymous.stdout.txt` (5980378 bytes, sha256 `c62e35bd30bde4cd621eb7701c0f58cb476a76b3421c963d00a2d8e988ed5eda`) — redacted and scanned before hashing; published as a release asset at publication
