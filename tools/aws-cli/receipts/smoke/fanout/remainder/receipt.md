# Smoke receipt — `aws-cli` / mode `s3api-v2-remainder`
...
| | |
| --- | --- |
| Date (UTC) | 2026-07-17T08:19:38Z |
| Exit code | `0` |
| Wall-clock | 0.611s (container lifetime, StartedAt→FinishedAt) |
...
| | | |
| --- | --- | --- |
| `peak_rss` | 76.4 MB | `VmHWM` of the container's main process, 10 successful samples. **Main process only** — a multi-process fan-out mode's children are not included. |
| `cgroup_peak_mem` | 61.8 MB | cgroup v2 `memory.peak`, whole container tree, 11 successful samples. **Page cache and kernel/socket memory included. Never present this as RSS.** |
...
- LIST requests (derived): **1** — root listing, delimiter=/ Contents-only (1 unprefixed key)
- Evidence: [INFERRED from ceil + [OBS --debug] `_capability/README.md`]

## Raw output

- stdout: inline — `stdout.txt` (87 bytes, sha256 `6303fb5a219ce05581cc0a4372bcc64f645756c675da627d1807f46957220f5b`)
...
**PASS** — `harness/verify-listing.sh` **union remainder** — this shard carries no single-receipt verdict; it is verified only as the designated `--remainder` in `../union/union-verify.md` (union PASS: 148917 distinct, 0 dup, remainder attributes the 1 unprefixed root key). Adapter: `tools/aws-cli/normalize.sh` mode `s3api-v2-remainder`.
