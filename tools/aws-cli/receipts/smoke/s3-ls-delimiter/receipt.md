# Smoke receipt — `aws-cli` / mode `s3-ls-delimiter`
...
| | |
| --- | --- |
| Date (UTC) | 2026-07-17T08:18:30Z |
| Exit code | `0` |
| Wall-clock | 0.647s (container lifetime, StartedAt→FinishedAt) |
...
| | | |
| --- | --- | --- |
| `peak_rss` | 78.0 MB | `VmHWM` of the container's main process, 11 successful samples. **Main process only** — a multi-process fan-out mode's children are not included. |
| `cgroup_peak_mem` | 63.3 MB | cgroup v2 `memory.peak`, whole container tree, 11 successful samples. **Page cache and kernel/socket memory included. Never present this as RSS.** |
...
- LIST requests (derived): **1** — root listing, delimiter=/ (4 CommonPrefixes + 1 key < 1 page)
- Evidence: [INFERRED from ceil + [OBS --debug] `_capability/README.md`]

## Raw output

- stdout: inline — `stdout.txt` (238 bytes, sha256 `b037077b276329f70153ad5982e9072f0ba74b22284aaa71073e5f880c8be592`)
...
**PASS** — `harness/verify-listing.sh` scope `delimiter=/ prefix=<none>`, expected=5, distinct=5, dups=0, missing=0, extra=0, field-mismatches=0. Verified against manifest sha256 `c78a82737dd1982a999912afa89f870c013cb22e01e50b8c4835ddb725992adb` (snapshot 2026-07-17). Adapter: `tools/aws-cli/normalize.sh` mode `s3-ls-delimiter`.
