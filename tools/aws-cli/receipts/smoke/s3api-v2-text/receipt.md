# Smoke receipt — `aws-cli` / mode `s3api-v2-text`
...
| | |
| --- | --- |
| Date (UTC) | 2026-07-17T08:13:06Z |
| Exit code | `0` |
| Wall-clock | 26.061s (container lifetime, StartedAt→FinishedAt) |
...
```sh
docker run -d --network host -e AWS_EC2_METADATA_DISABLED=true -e TZ=UTC -e AWS_ACCESS_KEY_ID= -e AWS_SECRET_ACCESS_KEY= -e AWS_SESSION_TOKEN= -e AWS_SECURITY_TOKEN= -e AWS_CONTAINER_CREDENTIALS_RELATIVE_URI= -e AWS_CONTAINER_CREDENTIALS_FULL_URI= -e AWS_CONTAINER_AUTHORIZATION_TOKEN= -e AWS_ROLE_ARN= -e AWS_SHARED_CREDENTIALS_FILE=/nonexistent-by-harness -e AWS_CONFIG_FILE=/nonexistent-by-harness -e AWS_WEB_IDENTITY_TOKEN_FILE=/nonexistent-by-harness amazon/aws-cli@sha256:406ca32d31e640a56e8d52921b40528cc64bfa59ec9cb4ee1456db6746cb7292 s3api list-objects-v2 --bucket noaa-normals-pds --region us-east-1 --no-sign-request --query Contents\[\].\[Key\,Size\,ETag\,LastModified\,StorageClass\] --output text
```
...
| | | |
| --- | --- | --- |
| `peak_rss` | 79.9 MB | `VmHWM` of the container's main process, 480 successful samples. **Main process only** — a multi-process fan-out mode's children are not included. |
| `cgroup_peak_mem` | 66.2 MB | cgroup v2 `memory.peak`, whole container tree, 481 successful samples. **Page cache and kernel/socket memory included. Never present this as RSS.** |
...
- LIST requests (derived): **149** — 148,917 objects
- Evidence: [INFERRED from ceil + [OBS --debug] `_capability/README.md`]

## Raw output

- stdout: external — `receipts/aws-cli/s3api-v2-text.noaa-normals-pds.full.anonymous.stdout.txt` (18547130 bytes, sha256 `50f157fc74c6a06f3ebd9bc3bfa6f093bc4f4ba3cf99db6b862637791d270388`) — redacted and scanned before hashing; published as a release asset at publication
...
**PASS** — `harness/verify-listing.sh` scope `full`, expected=148917, distinct=148917, dups=0, missing=0, extra=0, field-mismatches=0. Verified against manifest sha256 `c78a82737dd1982a999912afa89f870c013cb22e01e50b8c4835ddb725992adb` (snapshot 2026-07-17). Adapter: `tools/aws-cli/normalize.sh` mode `s3api-v2-text`.
