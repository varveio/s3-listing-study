...
- `src/benchmark/` is the importable package — the only part of this directory
  ...
  - `campaign.py` exposes one submission interface and records attempts in
    `campaign.db`; it owns the GCP Batch lifecycle and can run a bounded,
    synchronous Docker session for the current real-S3 experiment.
...
Logical plan rows are independent of execution location. Select the execution
path on the same `campaign.py submit` interface. For the current host:
...
Docker path runs synchronously and serially and is currently qualified only for
