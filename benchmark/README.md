...
- `build/` holds the toolbox build inputs: `Dockerfile` builds one linux/amd64
  toolbox directly from the ten active checked-in capsule recipes (it consumes no
...
and smokes all ten active executables. Retired capsules remain as historical
research but do not enter the toolbox. That is harness/toolbox smoke. The committed
