...
def _commands(attempt: Attempt, config: BatchConfig) -> list[str]:
    ...
    pairs = (
        ...
        ("--timeout", str(attempt.case.timeout_s)),
        ("--term-grace", str(config.term_grace_s)),
        ("--tool", attempt.case.tool),
        ("--tool-version", _image_token(attempt, "tool_version")),
        ("--shared-base-digest", str(attempt.image["shared_base_digest"])),
        ("--shared-base-uri", str(attempt.image["shared_base_uri"])),
        ("--derived-image", str(attempt.image["derived_image"])),
        ("--harness-revision", _image_token(attempt, "harness_revision")),
        ("--operation", "list"),
        ("--auth", attempt.case.auth),
        ("--mode", attempt.case.mode),
        ("--bucket", attempt.bucket),
        ("--region", attempt.region),
        ("--prefix", ""),
        ("--scope", "full"),
        ("--campaign-id", attempt.campaign),
        ("--job-id", attempt.job_id),
        ("--case-id", attempt.case.case_id),
        ("--case-fingerprint", attempt.case.fingerprint),
        ("--attempt-fingerprint", attempt.fingerprint),
        ("--run-ordinal", str(attempt.run_ordinal)),
        ("--submission-number", str(attempt.submission)),
        ("--machine-type", resources.machine_type),
        ("--vcpus", str(resources.vcpus)),
        ("--memory-gb", str(resources.memory_gb)),
        ("--container-memory-gb", "none" if container_memory is None else str(container_memory)),
        ("--destination", f"gs://{config.results_bucket}/{attempt.prefix}"),
    )
