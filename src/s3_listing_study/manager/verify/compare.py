...
(:mod:`s3_listing_study.manager.contract`), but a DuckDB ``VARCHAR`` is UTF-8 text and
...
def bad_mtimes(rows: Iterable[Row]) -> list[bytes]:
    ...
    :func:`~s3_listing_study.manager.contract.canon_mtime` keeps digits only, so it also
