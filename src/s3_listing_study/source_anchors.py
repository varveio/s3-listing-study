...
@overload
def git(root: Path, *args: str, text: Literal[True] = True) -> subprocess.CompletedProcess[str]: ...


@overload
def git(root: Path, *args: str, text: Literal[False]) -> subprocess.CompletedProcess[bytes]: ...
    ...
    for page in pages:
        ...
        for match in SRC_LABEL.finditer(text):
            ...
            for segment in body.split(";"):
                ...
                lines = (
                    re.sub(r"\s+", "", part.group("lines"))
                    if part and part.group("lines")
                    else None
                )
...
TreeCache = dict[tuple[str, str], set[str]]
BlobCache = dict[tuple[str, str, str], int | str]


def tree_files(root: SourceRoot, commit: str, cache: TreeCache) -> set[str]:
...
def file_lines(root: SourceRoot, commit: str, path: str, cache: BlobCache) -> int | str:
...
def check_anchor(anchor: Anchor, root: SourceRoot, path: str, cache: BlobCache) -> str | None:
...
def run(anchors: list[Anchor], roots: list[SourceRoot]) -> Outcome:
    ...
    for source_root in roots:
        source_root.used = False
    for anchor in anchors:
        chosen_root = resolve_root(anchor, roots)
        if chosen_root is None:
            key = (anchor.repository or "(no repository cited)", anchor.commit)
            skipped[key] = skipped.get(key, 0) + 1
            continue
        chosen_root.used = True
        if chosen_root.is_at(anchor.commit) and anchor.repository:
            ...
            for candidate in roots:
                if identity in candidate.identities:
                    candidate.used = True
        if not chosen_root.is_at(anchor.commit):
            refusal_key = (
                str(chosen_root.path),
                chosen_root.head,
                anchor.repository or "cited",
                anchor.commit,
            )
            refused[refusal_key] = refused.get(refusal_key, 0) + 1
            continue
        resolved_path: str | None = anchor.path
        if anchor.best_effort:
            resolved_path, unresolvable = resolve_best_effort_path(anchor, chosen_root, trees)
            if resolved_path is None and not unresolvable:
        ...
        reason = check_anchor(anchor, chosen_root, resolved_path, blobs)
        if reason:
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="s3-listing-study check-source-anchors")
